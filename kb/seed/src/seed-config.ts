 ...
 *
 * Publica en `config/app` las reglas de recomendación del repo y la versión del
 * catálogo. Es el hermano del seed de alimentos: mismo cliente REST, misma
 * idempotencia por comparación, mismos flags. Lo que cambia es el destino —un
 * único documento compartido en vez de una colección— y por eso escribe con
 * MERGE: los campos de otra mano (`copy`, `max_scans_per_day`) no se tocan.
 *
...
Qué hace
  - Valida el documento de reglas ANTES de tocar la red: la lista cerrada de
    tags, que el tag de cada regla y el fallback_tag pertenezcan a ella, y que
    ninguna condición nombre un identificador que el motor no conoce.
  - Escribe en config/app, con MERGE de cuatro campos:
    ...
    Todo lo demás que tenga el documento (copy, max_scans_per_day, …) queda
    intacto: la máscara de campos no lo alcanza.
...
async function principal(): Promise<void> {
  ...
  // Las dos lecturas de disco van ANTES que la red: un documento de reglas
  // inválido tiene que frenar la corrida sin haber abierto una conexión.
  ...
  const resultado = await correrSeedConfig(cliente, reglas, catalogo.kb_version, {
    seco: opciones.seco,
  });
  reportar(resultado, reglas, catalogo.kb_version, destino.descripcion, opciones.reglas);
}
