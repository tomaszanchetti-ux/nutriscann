...
| **2.4 — Golden set + Q/A E2E** | 10 fotos de platos conocidos con resultados esperados; el circuito local entero medido. **Abre con el E2E real de la primera foto (Bloque 0 de la WS05)** | ⬜ WS05 |
...
**Dónde retoma la WS05:** (1) E2E real — levantar emuladores + `npm run kb:seed:local` + front, y analizar la primera foto de verdad (la key local ya está verificada contra la API); (2) card 2.4 (golden set); (3) DT-18 (`copy` al seeder — 🔴 antes del cierre de fase); (4) deploy real: DT-2 (key a Secret Manager) + DT-3 (Storage) + ampliar el target del CI. El seed real del catálogo 3.0.0 a Firestore (3 deprecaciones) también queda para el cierre, con OK de Tomás.
