...
**Dónde retoma la WS06:** (1) **DT-28** — los mínimos del golden v2 (cola descriptiva USDA en el índice · tokenizar `/` · el payload de la compuerta · recalibrar el criterio 2 · medir estabilidad de la visión) y re-corrida para buscar 5/5; (2) **deploy real** (diferido a propósito desde la WS05): DT-2 (key a Secret Manager, la carga Tomás) + DT-3 (Storage) + ampliar el target del CI + **seed real del catálogo 3.1.0 a Firestore con OK de Tomás** (incluye el primer seed de config: reglas + 18 textos + kb_version); (3) DT-27 si hay tiempo — el lote de fichas nuevas con los insumos de Tomás ya documentados en DEUDAS (cerveza con porciones de barra, limón, tortilla de maíz, arepa por receta, tragos). Además Tomás trae su listado de comentarios de front para la Fase 3.
