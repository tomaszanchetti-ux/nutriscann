...
**Dónde retoma la WS06 (agenda confirmada por Tomás al cierre de la WS05):** todo back + Q/A — (1) **DT-28**: los mínimos del golden v2 (cola descriptiva USDA en el índice · tokenizar `/` · el payload de la compuerta · recalibrar el criterio 2 · medir estabilidad de la visión) y re-corrida buscando **5/5**; (2) **DT-27**: el lote de fichas nuevas con los insumos de Tomás ya documentados en DEUDAS (cerveza con porciones de barra completas, limón, tortilla de maíz, arepa por receta compuesta, tragos); (3) **los comentarios de front de Tomás** — se toman y se convierten en las cards de una Fase 3 simple. **El deploy (DT-2 + DT-3 + target del CI + seed real 3.1.0+ con OK de Tomás) pasa al cierre de la Fase 3**, decisión de Tomás del 01/09.
