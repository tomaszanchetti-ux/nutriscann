...
>
> **Estado:** Fase 0 cerrada. Fase 1 COMPLETA y mergeada (WS03). **Fase 2 BACKEND CERRADO — WS06 (01/09/2026): el golden pasa 5 de 5 criterios de mercado.** Cards 6.1–6.6 ejecutadas en multiagente con Q/A adversarial: DT-28 (motor: colas descriptivas EN/ES, barra `/`, compuerta coherente, criterio 2 recalibrado, estabilidad de visión medida — 17/30 platos se mueven entre corridas) · DT-27 y el mediterráneo completo (catálogo **3.5.0: 1.114 fichas**, +92 en la WS: cerveza con barra de España, salmón ×4, 44 recetas compuestas, calçots con transform propio medido; censo de 141 platos con candado: **86 ok, 0 equivocados, 4 ausentes** — 3 esperan BEDCA, 1 el modelo de retención DT-36) · **golden v3: 5/5 criterios** (v1 1/5 → v2 3/5), 24✅/6🟡/0❌, 0 errores de aritmética en 68 ítems, USD 0,26/corrida · Fase 3 con 5 cards definidas + tiers premium cerrados (§6.7). El golden set vive en `golden/` del repo con replay offline. **Branch `fase/02-motor-analisis` pusheada SIN merge** (merge solo con OK de Tomás). Ver "Dónde retoma la WS07".
>
...
**Dónde retoma la WS07 — Fase 3 (front + deploy):** las 5 cards ya definidas en §Fase 3. Antes de arrancar, el **repaso barato de deudas de motor/curación** que la v3 dejó servido: DT-39 (el alias `Jamón ibérico` de UNA línea → criterio 2 a 92,3 %) · DT-37 (la compuerta y el duelo de idiomas) · DT-38 (parches al criterio) · DT-32 (las guardas que no muerden) · la pasada BEDCA de DT-35 (besugo, perdiz, halloumi). **El deploy (DT-2 + DT-3 + target del CI + seed real 3.5.0 con OK de Tomás) es la card 3.5, cierre de la Fase 3** — decisión de Tomás del 01/09.
