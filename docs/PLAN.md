...
**Dónde retoma la WS08 — el Q/A visual de Tomás y la card 3.5 (deploy, cierre de la Fase 3):** la WS07 dejó la instancia local levantada (emuladores + front) para el Q/A visual de Tomás sobre las cards 3.1–3.4; los ajustes que salgan de ahí se ejecutan primero. Después la **card 3.5**: DT-2 (la key la carga Tomás) + DT-3 (Storage) + target completo del CI + **seed real con OK de Tomás** — que ahora publica el catálogo 3.8.0 CON las 21 guardas (sin re-seed, prod corre con 2 de arranque en frío), las 47 claves de copy (29 nuevas), los 3 scanning_steps nuevos + el 4.º paso pendiente ('Preparando la foto'), y la mudanza de `copy.premium.ts`/`copy.terminos.ts` si se decide — + verificación E2E desde el teléfono de Tomás. Decisiones abiertas de Tomás: confianza del `Jamón ibérico` (0,6 vs 0,8) · cláusula del 70 % del criterio · los textos de T&C y de la vitrina · la marca (`nutriscan.app` existe) · DT-40 (los errores del backend que vosean).
