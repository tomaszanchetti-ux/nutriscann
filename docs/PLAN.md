...
>
> **Estado (WS13, 03/09/2026) — Fase 6, branch `fase/06-gramos-y-confianza` SIN merge.** Cards **6.1** (confianza ponderada por gramos + eslabón más débil declarado) y **6.3** (desempate crudo/cocido por familia) **CERRADAS con Q/A adversarial** (informes en `docs/fase6.card-6.x.md`; script nuevo `golden/bin/replay-vision.js`). **6.2 REDEFINIDA y cerrada por decisión de Tomás: el total se publica SIEMPRE** — la compuerta del 12 % deja de apagar la ficha, porque el score de confianza no mide acierto sino la vía del match (el golden dio 0 fichas equivocadas con platos correctos a 0,2–0,4); `total_no_publicable` desaparece. **T&C alineados (DT-59 ✅)**: cuenta y cupo, qué se hace con las fotos, la opinión por ítem, la v1.5 y los gramos aproximados; `soporte@caliscan.app` vivo (DT-67 ✅). **Prioridades fijadas por Tomás: siempre hay ficha + gramos precisos al menor costo; 6.4, 6.7 y el plástico (DT-71) esperan.** Suite: 583 tests en functions, 126 en el seed. Opus 5 en 529 → Sonnet 5 cerró todo. **Dónde retoma la WS14:** (1) **6.2b píldoras por vía** — cuatro categorías calculadas en el BACKEND (🟢 Exacto · 🟡 Muy parecido · 🟠 Aproximado · 🔴 Estimación), el compuesto hereda la de su peor ingrediente, el plato la de su peor ítem; (2) **Bloque 0 de gramos**: relevar `NutriScann/Calculo de Gramos/*.pdf` y https://aeeh.es/liverai-pesos-raciones/, cruzarlo con los `portion_hints` y con los `components` que ya descompone Sonnet → 6.5; pedir a Tomás fotos de peso conocido para la 6.6; (3) al desplegar, re-seed de `config/copy.json` si la 6.2b cambia claves. Deudas nuevas DT-67…DT-71.
>
> *(Registro WS12, superseded.)* **Estado (WS12, 03/09/2026) — Fase 6 «La confianza dice la verdad y los gramos se cuentan», branch `fase/06-gramos-y-confianza` SIN merge.** Bloque 0 en `docs/bloque0.fase6.md`: el E2E de Tomás (3 fotos) más 10 expedientes de producción de 3 usuarios y el golden re-jugado con la visión v6. Hallazgos: la confianza de un compuesto es la del PEOR ingrediente ×0,8 (la ensalada de atún, seis fichas correctas, salió sin total a 0,107); la compuerta del 12 % se calibró sin compuestos y hoy frena a la ensalada buena y deja pasar a la comida de plástico (1.098 kcal, DT-65); un empate crudo/cocido lo decidía el largo del nombre (lechuga y tomate «cocidos» en toda ensalada, +12 % de kcal); y las 1.115 fichas ya traen pesos por porción que nadie usa para estimar gramos. **Card 6.0 CERRADA con Q/A** (dos commits: el expediente guarda `vision` byte a byte y la foto en Storage, la foto se queda si y solo si quedó escrito su documento; 594 tests, 0 skip). **6.1 y 6.3 a mitad**, guardadas en un commit WIP explícito porque la API de Opus se sobrecargó: la WS13 las retoma desde `git show` de ese commit. Cards definidas: 6.0–6.7 (tabla en el Bloque 0). 🔴 **DT-59 bloquea el merge: los T&C tienen que decir que se guarda la foto.** Deudas nuevas DT-59…DT-66.
>
> *(Registro WS11, superseded.)* **Estado (WS11, 03/09/2026) — Fase 5 «Números que cierran y motor que siempre responde», branch `fase/05-motor-responde` SIN merge.** Bloque 0: los 4 escaneos reales del 02/09 dieron tres fallos distintos (el 102 % de carbos era aritmética; la composición nunca disparaba; el canal inglés es el débil, no el español). Cards cerradas con Q/A: **5.1** el reparto del donut suma 100 por construcción (74 fichas publicaban >100 %); **Bloque 0 de familias** (46 familias / 191 subfamilias, `kb/curation/familias.json`, informe `kb/cobertura/familias.bloque0.md`, 94,6 % del catálogo alcanzable sin llamada extra); **5.2** la visión devuelve `familia_subfamilia` de una lista cerrada, descompone siempre y lee etiquetas de envase (golden 30 fotos: 47/53 en la subfamilia esperada); **retoque de costo**: prefijo 16.088 → 8.361 tokens, caché de 1 h, $0,108/usuario/mes a 15 escaneos calientes; **5.3** cascada nombre → sustituto → componer → cabeza de subfamilia → cabeza de familia → composición parcial, con el **candado de plausibilidad** para toda ficha construida (las 1.115 lo pasan). Golden re-jugado: 0 fichas cambiadas. La pizza del 02/09 sale con 401 kcal. 542 tests en functions, 126 en el seed. **Dónde retoma la WS12:** Q/A visual de Tomás sobre la fixture (`VITE_ANALYZE_FIXTURE=1` y `=completo`), merge con su OK, deploy por CI y re-seed de `config/copy.json` (114 claves); deudas nuevas DT-51…DT-58.
>
