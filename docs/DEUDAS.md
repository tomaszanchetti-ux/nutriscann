...
| DT-28 | 🔴 abierta | **Los mínimos del golden v2** (re-corrida 01/09: 3 de 5 criterios pasan, 0 fichas equivocadas; esto es lo que falta): (1) normalizar la **cola descriptiva larga de USDA** en el índice (`fat added, NS as to fat type`, `with peel`, `NS as to part and cooking method` — el motor ya normaliza `NFS`/`NS as to fat` pero no estas) → arregla 5 de los 9 silencios restantes y con eso los criterios 1 y 5 a la vez · (2) tokenizar `/` en los términos de la visión (`serrano/iberico` rompió el alias del jamón) · (3) **`totals.nutrients` no debe viajar con número cuando la compuerta cierra** (el plástico declara `completo:false` y motivo, pero las 1.550 kcal siguen en el payload) · (4) recalibrar el criterio 2 del golden set contra los gramos QUE LA VISIÓN REPORTA, no los predichos (5 de los 6 platos "fuera de rango" tienen ficha correcta y aritmética exacta: juzgado así daría 96,2 %) · (5) **medir estabilidad de la visión**: 12 de 30 platos cambiaron ítems o gramos ±30 % entre corridas sobre las MISMAS fotos — al menos 3 juicios los decidió la visión, no el motor | Claude (WS06, antes del deploy) |
| DT-27 | 🟡 diferida | **Lote de fichas nuevas v1.1** (huecos reales verificados por el golden test de 30; son fichas, no vocabulario — entran por selección USDA o curación manual con fuente declarada): 🍺 **cerveza priorizada por Tomás (01/09)** — 2-3 fichas desde USDA SR (industrial ~43 kcal/100ml · artesanal/alta graduación · sin alcohol), con aliases y porciones de España aportados por Tomás (caña/doble/tercio/pinta) · **pechuga de pollo** (no existe en las 1.022; solo "Pollo guisado" genérico) · limón entero · tortilla de maíz · pita · alubias en salsa de tomate · arepa | Tomás (producto) + Claude (WS06) |
