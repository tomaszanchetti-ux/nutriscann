#!/usr/bin/env node
/**
 * vision-v6.js — la visión del RETOQUE de la card 5.2, contra las mismas 30 fotos.
 *
 * Es el mismo script que `vision-v5.js` y a propósito: la única forma de saber si
 * el prompt achicado perdió precisión es correr LAS MISMAS fotos con LA MISMA
 * medición y comparar renglón por renglón. Lo único que cambia acá es el destino
 * por defecto (`set-30/vision-v6/`); el prompt, el esquema y el caché los pone
 * `functions/lib/analyze/vision.js`, o sea producción.
 *
 * Qué se está midiendo, y contra qué:
 *   · la v5 dio 47/53 ítems en la subfamilia esperada, 58 componentes en 14 fotos
 *     y 3 etiquetas de envase, con un prefijo de sistema de 16.088 tokens;
 *   · la v6 tiene que dar lo mismo con 8.294. Si pierde más de 2 ítems de
 *     subfamilia, la lista compacta se quedó corta y hay que devolverle
 *     información (y contar cuánto costó en tokens).
 *
 * CUESTA PLATA: son 30 llamadas al modelo. Ningún test lo invoca y ningún CI lo
 * corre; se corre a mano, una vez, y lo que deja grabado se puede leer todas las
 * veces que haga falta sin gastar un centavo más. Por eso se PLANTA si el
 * directorio de salida ya tiene respuestas.
 *
 * Y hay una lectura más que en la v5 importa mirar: `cache_write` tiene que
 * aparecer en la PRIMERA foto y en ninguna otra, y `cache_read` en las 29
 * restantes. Si el write reaparece a mitad de la corrida, el prefijo está
 * variando entre llamadas y el caché de 1 hora no está sirviendo para nada.
 *
 * Uso:
 *   node golden/bin/vision-v6.js                 # las 30
 *   node golden/bin/vision-v6.js -f '3.'         # solo las que matcheen
 *   node golden/bin/vision-v6.js --forzar        # sobreescribe lo grabado
 *
...
const salida = (args.find((a) => a.startsWith("--salida=")) ?? "--salida=vision-v6").split("=")[1];
